...
class Lyric{
    ...
    parseLrc(lrc_content){                                      // 解析歌词 
        ...
        if(custom_index == ~lyric_list.length){
            lyric_obj[0] = 'Sorry。这首歌的歌词无法自动滚动。';
            lyric_obj.unable_scroll = 'end';
        }
    ...
    renderLrc(lyric_obj){               // 将歌词渲染到页面中
        ...
        for(let lyric in lyric_obj){
            ...
            } else if(lyric_obj.unable_scroll){
                li.className = "unable-scroll-lyric";
            } else {
                li.className = "lyric";
            }
    ...
    requestAnimation(data,end){            // 滚动滚动条而使歌词滚动
        let run = () => {
            ...
            if((Math.abs(parseInt(end - this.lrc_container.scrollTop)) > Math.abs(data))&&      
                (Math.ceil(this.lrc_container.scrollTop+this.lrc_container.clientHeight)<this.lrc_container.scrollHeight)){ //滚动条滚动到底部

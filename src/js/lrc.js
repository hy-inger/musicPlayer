...
function scrollLrc(currentTime){
    let lyric_li = lrc_list.find('.lyric'),
        index = lyric_index[currentTime];       // 获取当前时间的歌词下标
    if(!index){         // 当前时间没有匹配的歌词
        return;
    }
    let active_lyric = $q('.lrc-list .active'),
        active_index = active_lyric.attr("data");
    if(active_index == index){      // 该句歌词已滚动过不再滚动
        return;
    }
    if(index > 5 && index < (lyric_li.length - 2)){

...
function renderLrc(lyric_obj){
    ...
    for(let lyric in lyric_obj){
        ...
        lyric_time[k] = parseInt(lyric);                // 记录歌词时间
        k++;
    }
...
function requestAnimation(data,end){
    let run = function(){
        ...
        if(Math.abs(parseInt(end - lrc_container.scrollTop)) > Math.abs(data)){
            ...
            lrc_container.scrollTop += (end - lrc_container.scrollTop);
...
function scrollTopLrc(currentTime){
    ...
    if(index < (lyric_height.length - 2)){
        let active_height = lyric_height[index],
...
/* 点击歌词事件，更新audio的currentTime */
lrc_list.on('click',function(e){
...
$q('.progress').on('mousemove',function(e){
    ...
    let width = _this[0].offsetWidth,
            per = left/width;
    drag_time = parseInt(per * lyric_time[lyric_time.length-1]);
...
function dragToLrc(per){
    let current_time = 0;
    if(!lyric_index[drag_time]){
        lyric_time.push(drag_time);
        lyric_time.sort(number);
        for(let index in lyric_time){
            if(lyric_time[index] == drag_time){
                current_time = lyric_time[index-1];
                lyric_time.splice(index,1);
                break;
            }
        }
    ...
    audio[0].currentTime = current_time;
}